...
test('works', () => {
  ...
  const firstReply = controller.addTaggedValue(
    first,
    'comment',
    'reply to first'
  );
  ...
  const firstReplyReply = controller.addTaggedValue(
    firstReply,
    'comment',
    'reply to reply to first'
  );
  controller.addTaggedValue(
    firstReplyReply,
    'comment',
    'reply to reply to reply to first'
  );

  expect(Array.from(comments.values()).map(x => x.getValue())).toEqual([
    'FIRST',
    'do not reply to FIRST'
  ]);

  expect(
    Array.from(first.getOwnedByTag('comment').values()).map(x => x.getValue())
  ).toEqual(['reply to first']);

  expect(Array.from(allComments.values()).map(x => x.getValue())).toEqual([
    'FIRST',
    'reply to first',
    'do not reply to FIRST',
    'reply to reply to first',
